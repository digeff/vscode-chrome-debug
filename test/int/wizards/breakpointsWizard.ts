...
class EventAvailableToBeConsumed implements IEventsConsumptionState {
    public constructor(_changeState: ChangeState, public readonly latestEvent: DebugProtocol.StoppedEvent | DebugProtocol.ContinuedEvent) { }
...
export class FileBreakpointsWizard {
    ...
    public async hitCountBreakpoint(options: { lineText: string; hitCountCondition: string; }): Promise<BreakpointWizard> {
        return (await (await this.unsetHitCountBreakpoint(options)).setAndVerify()).assertIsVerified();
    }
