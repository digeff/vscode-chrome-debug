...
class BatchingUpdates implements IInternalFileBreakpointsWizardState {
    ...
    public async processBatch(): Promise<void> {
        const remainingBreakpoints = _.difference(Array.from(this.currentBreakpointsMapping.keys()), this._breakpointsToSet.toArray(), this._breakpointsToUnset.toArray());
        await this._internal.sendBreakpointsToClient(new BreakpointsUpdate(Array.from(this._breakpointsToSet), Array.from(this._breakpointsToUnset), remainingBreakpoints));
    }

    public assertIsVerified(_breakpoint: BreakpointWizard): void {
...
export class InternalFileBreakpointsWizard {
    ...
    public async batch(batchAction: (fileBreakpointsWizard: FileBreakpointsWizard) => PromiseOrNot<void>): Promise<void> {
        ...
        await batchingUpdates.processBatch();
    }
