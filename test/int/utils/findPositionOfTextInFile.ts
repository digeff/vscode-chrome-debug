...
export async function findPositionOfTextInFile(filePath: string, text: string): Promise<Position> {
    const contents = await utils.readFileP(filePath, 'utf8');
    ...
    if (textStartIndex >= 0) {
        ...
        const textColumNumber = createColumnNumber(textStartIndex - lastNewLineBeforeTextIndex);
        return new Position(textLineNumber, textColumNumber);
    } else {
        throw new Error(`Couldn't find ${text} in file at: ${filePath}`);
    }
}
